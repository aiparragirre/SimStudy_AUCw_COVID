...
f.sim.real <- function(population, response.var,
  ...
  coefs <- unw.coefs <- matrix(NA, nrow = length(pop.eredua$coefficients), ncol = runs)
  colnames(coefs) <- colnames(unw.coefs) <- paste0("r = ", 1:runs)
  rownames(coefs) <- rownames(unw.coefs) <- names(pop.eredua$coefficients)
  ...
  for(r in 1:runs){
    ...
    if(length(unw.model$coefficients) == nrow(unw.coefs)){
      unw.coefs[,r] <- unw.model$coefficients  
    }
    model <- survey::svyglm(formula, design = mydesign, family = "quasibinomial")
    if(length(model$coefficients) == nrow(coefs)){
    ...
    # AUC of the sample model in the finite population
    try.pop.auc <- try(suppressMessages(pROC::roc(pop.eredua$y, predict(model, newdata = population, type = "response"))$auc[1]))
    if (inherits(try.pop.auc, 'try-error')) {
      pop.auc[r] <- NA
    } else {
      pop.auc[r] <- suppressMessages(pROC::roc(pop.eredua$y, predict(model, newdata = population, type = "response"))$auc[1])
    }
  ...
  out$coefs <- list()
  out$coefs$pop <- pop.eredua$coefficients
  out$coefs$unw <- unw.coefs
  out$coefs$w <- coefs
